...
class TranscriptDesigner:
    ...
    def _exhaustive_optimize(self, utr: str, codons: list[str], amino_acids: str) -> list[str]:
        """
        Iterates through the sequence and tries every synonymous alternative for each position.
        Prioritizes removing Forbidden Sequences and Internal Promoters above all else.
        """
        ...
        for i in range(len(current_codons)):
            ...
            for candidate in candidates:
                # Temporarily update the codon at position i
                original_at_i = current_codons[i]
                current_codons[i] = candidate
                test_seq = utr + "".join(current_codons)
                pos_in_seq = (i * 3) + len(utr)
                
                # Window for motif-based checkers (+/- 60bp is enough for promoters and enzymes)
                motif_window = test_seq[max(0, pos_in_seq - 60) : pos_in_seq + 60]
                
                # --- Hard Constraints (Safety) ---
                passed_forb, _ = self.forbidden_checker.run(motif_window)
                passed_promoter, _ = self.promoter_checker.run(motif_window)
                passed_homo, _ = self.homopolymer_checker.run(motif_window)
                
                # --- Soft Constraints (Quality) ---
                # Check hairpin in local context
                hairpin_window = test_seq[max(0, pos_in_seq - 100) : pos_in_seq + 100]
                passed_hair, _ = hairpin_checker(hairpin_window)
                
                # Global metrics (Diversity/CAI)
                passed_metrics, _, _, _ = self.codon_checker.run(current_codons)

                # Assign Tiers: 0=Perfect, 1=Safe but messy, 2=Forbidden/Promoter present
                if passed_forb and passed_promoter and passed_homo:
                    tier = 0 if (passed_hair and passed_metrics) else 1
                else:
                    tier = 2
                ...
                # Reset for next candidate evaluation
                current_codons[i] = original_at_i

            # Sort by Tier (ascending) then Weight (descending)
            scored_candidates.sort(key=lambda x: (x['tier'], -x['weight']))
            
            # Update the position with the winning candidate
    ...
    def run(self, peptide: str, ignores: set) -> Transcript:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Perform the priority-based exhaustive optimization
                optimized_codons = self._exhaustive_optimize(utr, committed_codons, full_peptide)
                
                # Final health check
                codons_above_board, codon_div, rare_count, cai_val = self.codon_checker.run(optimized_codons)
                if not codons_above_board:
                    # If metrics fail even after optimization, we try a new attempt (new RBS)
                    raise ValueError(f"Codon metrics failed: CAI={cai_val:.2f}, Div={codon_div:.2f}")
                
                return Transcript(selectedRBS, full_peptide, optimized_codons)
        ...
        final_optimized = self._exhaustive_optimize(utr, committed_codons, full_peptide)
        return Transcript(best_rbs, full_peptide, final_optimized)